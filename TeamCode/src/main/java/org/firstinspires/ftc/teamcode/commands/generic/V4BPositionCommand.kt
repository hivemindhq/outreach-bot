...
private const val MOVE_TIME = 500L //ms
private const val MOVE_TIME_DEFAULT = 1000L //ms

fun v4bPositionCommand(state: DepositSubsystem.V4BState) = v4bPositionCommand(state, true)
fun v4bPositionCommand(state: DepositSubsystem.V4BState, withLock: Boolean): SequentialCommandGroup {
    ...
    val res = SequentialCommandGroup(
        InstantCommand({
            ...
            println("debug: (v4b [pos]) wait time = ${waitTime.get()}ms")
        }),
